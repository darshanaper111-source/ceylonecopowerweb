...
import {
  Battery, BatteryCharging, Building2, Factory,
  Home, Radio, ShieldCheck, Store, Sun, Zap,
} from "lucide-react";
import SectionTitle from "../components/SectionTitle.jsx";
import { serviceCategories } from "../data/services.js";

const ICON_MAP = { Factory, Building2, Sun, Store, Home, BatteryCharging, Radio, Zap, Battery };
      ...
      <div className="max-w-7xl mx-auto px-4 min-h-[calc(100vh-5rem)] grid md:grid-cols-2 gap-12 items-center py-20">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <Link to="/contact" className="bg-ecoGold text-black rounded-full px-8 py-3.5 font-bold hover:bg-yellow-400 transition">
              Request a Quote
            </Link>
            <Link to="/projects" className="border border-white/25 rounded-full px-8 py-3.5 font-bold hover:bg-white/10 transition">
              View Projects
            </Link>
          </div>
        </div>
        <div className="glass rounded-[2rem] p-5">
          <video autoPlay muted loop playsInline poster="/images/hero-photo.JPG" className="rounded-3xl w-full h-72 object-cover">
...
function ImageBanner() {
  return (
    <div className="relative w-full h-64 md:h-96 overflow-hidden">
      <img src="/images/hero-photo.JPG" alt="Ceylon Eco Power solar installation" className="w-full h-full object-cover" />
    ...
    <section className="bg-white text-ecoDark py-24">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ["Engineering First",   "Accurate solar design, cable sizing and protection coordination for every project."],
            ["Quality Components",  "Panels, inverters and batteries selected for long-term performance and reliability."],
            ["After-Sales Support", "Monitoring, troubleshooting, power quality analysis and scheduled maintenance."],
          ].map(([title, body]) => (
            <div key={title} className="bg-ecoCream rounded-3xl p-8 border border-black/10 shadow-lg">
...
function Services() {
  return (
    <section className="py-24 bg-ecoDark">
      <div className="max-w-7xl mx-auto px-4">
        <SectionTitle eyebrow="Solutions" title="Energy Solutions We Provide" subtitle="From home rooftops to utility-scale solar plants and off-grid mini grids." />
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {serviceCategories.map((svc) => {
            const Icon = ICON_MAP[svc.icon] || Zap;
            return (
              <div key={svc.id} className="rounded-3xl bg-white/5 border border-white/10 p-6 hover:border-ecoGold/60 transition">
                <Icon className="text-ecoGold" size={38} />
                <div className="mt-2 text-xs font-bold text-ecoGold/70 tracking-wider uppercase">{svc.range}</div>
        ...
        <div className="mt-12 text-center">
          <Link to="/projects" className="inline-block bg-ecoGold text-black rounded-full px-8 py-3.5 font-bold hover:bg-yellow-400 transition">
            See Our Projects
          </Link>
        </div>
      </div>
...
export default function HomePage() {
  return <><Hero /><ImageBanner /><About /><Services /></>;
}
